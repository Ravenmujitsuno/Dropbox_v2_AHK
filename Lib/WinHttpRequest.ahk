...
WinHttpRequest( URL, ByRef In_POST__Out_Data="", ByRef In_Out_HEADERS="", Options="" )
{
	...
	; SetTimer, __UpdateProgressBarDropBox, 100
	...
	; Progress, Off
	; SetTimer, __UpdateProgressBarDropBox, Off
	...
	; __UpdateProgressBarDropBox:
	; 	;Get the current filesize and tick
	; 	OutputDebug % "ResponseText = " oHTTP.ResponseText "`nResponsBody = " oHTTP.ResponseBody "`nResponse = " oHTTP.Response "`n"
	; 	CurrentSize := FileOpen(Destination, "r").Length ;FileGetSize wouldn't return reliable results
	; 	CurrentSizeTick := A_TickCount
	; 	;Calculate the downloadspeed
	; 	Speed := Round((CurrentSize/1024-LastSize/1024)/((CurrentSizeTick-LastSizeTick)/1000)) . " Kb/s"
	; 	;Save the current filesize and tick for the next time
	; 	LastSizeTick := CurrentSizeTick
	; 	LastSize := FileOpen(Destination, "r").Length
	; 	;Calculate percent done
	; 	PercentDone := Round(CurrentSize/FinalSize*100)
	; 	;Update the ProgressBar
	; 	Progress, %PercentDone%, %PercentDone%`% Done, Downloading...  (%Speed%), Downloading %Destination% (%PercentDone%`%)
	; Return
